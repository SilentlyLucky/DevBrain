...
function getDocumentIcon(doc: Document) {
  ...
  if (doc.source_type === "PDF" || ext === ".pdf")
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
      {Array.from({ length: state.numPages }, (_, i) => (
        <PdfPage key={i + 1} pdf={state.pdf} pageNum={i + 1} />
      ))}
    </div>
  );
}
          ...
          {rows.map((row, i) => (
            ...
            >
              {row.map((cell, j) => (
                <td
                  key={j}
                  style={{ border: "1px solid #ddd", padding: "7px 12px" }}
                >
                  {cell}
                </td>
              ))}
...
export function DocumentViewer({
  ...
  const usePdfCanvas =
    (doc.source_type === "PDF" || ext === ".pdf") && !!doc.file_url;
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const c = await fetchDocumentContent(doc.id);
  ...
  const useMd =
    ext === ".md" ||
    (doc.source_type === "URL" &&
      /^#{1,6}\s|^\*\*|^\- |\*[^*]/.test(resolvedContent));

  const wordCount = resolvedContent.trim()
    ? resolvedContent.trim().split(/\s+/).length
    : 0;

  const { Icon, color } = getDocumentIcon(doc);

  const sourceLabel =
    doc.source_type === "PDF" ? "PDF Document" : doc.source_type === "URL" ? "Web Article" : ext ? ext.slice(1).toUpperCase() + " File" : "Document";
  ...
  return (
    ...
    >
      ...
      {/* Content */}
      <div
        className="flex-1 overflow-y-auto py-10 px-4"
        style={{
          background: "var(--color-surface)",
        }}
      >
        {usePdfCanvas ? (
          ...
          >
            ...
            <div
              className="flex items-center gap-3 text-xs mb-8 pb-6"
              style={{
                color: "var(--color-muted-foreground)",
...
export function DocumentTable({ documents }: { documents: Document[] }) {
  ...
  function getDownloadFilename(doc: Document) {
    ...
    const ext =
      doc.file_extension ?? (doc.source_type === "PDF" ? ".pdf" : ".txt");
  ...
  async function downloadDocument(doc: Document) {
    try {
      let blob: Blob;
      const isBinary =
        doc.source_type === "PDF" || doc.file_extension === ".docx";
  ...
  async function handleTitleSave() {
    ...
    if (!nextTitle || nextTitle === editingDoc.title) {
      setEditingDoc(null);
      return;
    }
      ...
      <div className="rounded-xl border border-muted overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {documents.map((doc) => {
              const { Icon, color } = getDocumentIcon(doc);
              const typeLabel = doc.file_extension
                ? doc.file_extension.slice(1).toUpperCase()
                : doc.source_type;
              return (
                <tr
                  key={doc.id}
                  className="border-b border-muted last:border-0 hover:bg-muted/30 transition-colors"
                >
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Update the title shown in your Knowledge base.
            </DialogDescription>
          </DialogHeader>
