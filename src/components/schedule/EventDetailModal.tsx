...
import { X, Clock, FileText, Check, Trash2, Tag, Pencil, ArrowRight, Calendar } from 'lucide-react'
...
function parseDescription(desc: string | null | undefined): { category: string | null; text: string } {
...
export function EventDetailModal({ event, onClose }: Props) {
  ...
  async function handleSave() {
    ...
    if (!editStart || !editEnd) { toast.error('Start and end time are required'); return }
    if (new Date(editEnd) <= new Date(editStart)) { toast.error('End time must be after start time'); return }
  ...
  async function handleStatus(status: 'Completed') {
    setLoading(true)
    try {
      await updateScheduleStatus(event!.id, status)
  ...
  async function handleDelete() {
    const confirmed = await toast.confirm(`Delete "${event!.title}"? This cannot be undone.`)
    if (!confirmed) return
    setLoading(true)
    try {
          ...
          <div className="flex-1 min-w-0 pr-4">
            ...
            {!isEditing && category && (
              <div className="flex items-center gap-1.5 mt-2">
                ...
                <span className="text-xs font-semibold tracking-wide text-violet-400 uppercase">{category}</span>
              </div>
            )}
          </div>
          <div className="flex items-center gap-2 shrink-0">
            {!isEditing && (
              <span className={cn('text-[10px] px-3 py-1 rounded-full border font-bold uppercase tracking-wider', STATUS_BADGE[event.status])}>
                {event.status}
              </span>
            )}
            {!isEditing && (
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <select
                  ...
                  onChange={e => setEditReminder(e.target.value === 'null' ? null : Number(e.target.value))}
                  className={`${inputCls} h-10`}
                >
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 bg-surface rounded-xl p-3 border border-muted flex flex-col shadow-sm relative overflow-hidden">
                  ...
                  <span className="text-xl font-black text-foreground tracking-tight pl-1">{fmtTime(event.end_time)}</span>
                </div>
              </div>
              ...
              {descText && (
                <div className="rounded-xl p-4 mt-2 bg-surface border border-muted shadow-sm">
                  ...
                  <p className="text-sm leading-relaxed text-muted-foreground whitespace-pre-wrap">{descText}</p>
                </div>
              )}
            </div>
